...
uses
  ...
  uCEFChromium, uCEFApplication, uCEFWindowParent, uCEFInterfaces, uCEFConstants, uCEFTypes,
  uCEFWinControl;
...
begin
 if Selected then Button.ColorNormal:=$00B56D3E
 else Button.ColorNormal:=$009A572B;
end;
...
begin
 ...
 if not IndexInList(TableExList.ItemIndex, FKinoList.Count) then
  begin
   if AskYesNo('Вопрос', 'Элемент не выбран. Открыть просто сайт Кинопоиск?')
   then Str:='https://www.kinopoisk.ru';
  end
 else
  begin
   if FKinoList[TableExList.ItemIndex].KID > 1
   then Str:='https://www.kinopoisk.ru/film/'+IntToStr(FKinoList[TableExList.ItemIndex].KID)
   else Str:='https://www.kinopoisk.ru/index.php?kp_query='+FKinoList[TableExList.ItemIndex].Caption;
  end;
 ...
 else ShellExecute(Application.Handle, 'open', PChar(Str), nil, nil, SW_NORMAL);
end;
  ...
  begin
   if TryStrToInt(StringGridXL.Cells[7, i], r) then
    begin
     ...
     if not TryStrToInt(Copy(StringGridXL.Cells[2, i], 1, 4), y) then y:=FKinoList[s].Year;
     if (s >= 0) and (y = FKinoList[s].Year) then
      ...
      begin
       ...
       try
        ...
        if Item.Info <> '' then Item.Info:=Item.Info + ', ';
...
begin
 ...
 procedure TFormMain.ChromiumBeforePopup(Sender: TObject;
...
begin
 ...
 UpdateGlobalFilter;
 UpdateFilterNotify;
 if not Chromium.CreateBrowser(CEFWindowParent) then TimerChrome.Enabled:=True;
end;
...
begin
 ...
 if (not Chromium.CreateBrowser(CEFWindowParent)) and (not Chromium.Initialized) then TimerChrome.Enabled:=True;
end;
